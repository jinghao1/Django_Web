...
from apps.xfzauth.models import Project
...
class UserRegProject(View):
    ...
    def get(self, request):
        page = int(request.GET.get('p', 1))  # 获取当前所在页数
        title = request.GET.get('title')
        ProInfo = Project.objects.all().order_by("-id")
        # 过滤标题中包含指定关键字公司名称
        if title:
            ProInfo = ProInfo.filter(
                company_name__icontains=title)  # i指忽略大小写，contains指包含
        paginator = Paginator(ProInfo, 10)  # 将获取的新闻内容按照每页2篇的形式进行分页
        page_obj = paginator.page(page)  # 获取对应分页的数据
        # 通过分页函数返回分页数据，获取每一页的数据
        pagination_data = self.get_pagination_data(paginator, page_obj)
        '''查询内容组成的查询url是否应该带'''
        ...
        logger.info('用户查询了[%s]' % context['url_query'])
        context.update(pagination_data)
        return render(request, 'cms/project_list.html', context=context)